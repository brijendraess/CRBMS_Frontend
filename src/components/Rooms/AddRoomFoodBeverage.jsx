...
const AddRoomFoodBeverage = ({ room ,setRefreshPage,setIsFoodBeverageOpen}) => {
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post("api/v1/rooms/add-food-beverage", formData);
  ...
  return (
    <div className="pop-content w-100">
      ...
      >
        ...
        <InputLabel id="demo-multiple-name-label">Food Beverage Name</InputLabel>
        <Select
        ...
        >
          {FoodBeveragesList.map((foodBeverage)=><MenuItem value={foodBeverage.id}>{foodBeverage.label}</MenuItem>)}
        </Select>
